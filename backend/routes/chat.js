...
router.post("/:palId/message", async (req, res) => {
    try {
        ...
        Speaking Style: ${pal.personality.speakingStyle}. 
        
        Instructions: ${convo_instructions}`.trim();
...
// GET /api/chat/:palId/history
router.get("/:palId/history", async (req, res) => {
    try {
        const { palId } = req.params;
        const userId = '000000000000000000000000'; // Default user ID

        const history = await Chat.find({ palId, userId })
